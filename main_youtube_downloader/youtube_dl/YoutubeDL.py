...
class YoutubeDL(object):
    ...
    def __init__(self, params=None, auto_init=True):
        ...
        self._screen_file = [sys.stdout,
                             sys.stderr][params.get('logtostderr', False)]
        ...
        check_deprecated('autonumber', '--auto-number',
                         '-o "%(autonumber)s-%(title)s.%(ext)s"')
        check_deprecated('usetitle', '--title',
    ...
    def prepare_filename(self, info_dict):
        ...
        try:
            ...
            if autonumber_size is None:
                autonumber_size = 5
            template_dict['autonumber'] = self.params.get(
                'autonumber_start', 1) - 1 + self._num_downloads
            ...
            def sanitize(k, v): return sanitize_filename(
                compat_str(v),
            ...
            outtmpl = outtmpl.replace('%%', '%{0}%'.format(
                sep)).replace('$$', '${0}$'.format(sep))
            ...
            if sys.version_info < (3, 0) and sys.platform == 'win32':
                filename = encodeFilename(
                    filename, True).decode(preferredencoding())
        ...
        for ie in ies:
            ...
            try:
                ...
                if self.params.get('ignoreerrors', False):
                    self.report_error(error_to_compat_str(
    ...
    def _build_format_filter(self, filter_spec):
        ...
        if not m:
            ...
            if m:
                ...
                if m.group('negation'):
                    def op(attr, value): return not str_op(attr, value)
                else:
                    op = str_op
    ...
    def build_format_selector(self, format_spec):
            ...
            for type, string, start, _, _ in tokens:
                ...
                if type == getattr(tokenize, 'ENCODING', None):
                    ...
                    if string == ')':
                        ...
                        group = _parse_format_selection(
                            tokens, inside_group=True)
                        current_selector = FormatSelector(GROUP, group, [])
                    elif string == '+':
                        video_selector = current_selector
                        audio_selector = _parse_format_selection(
                            tokens, inside_merge=True)
                        if not video_selector or not audio_selector:
                            raise syntax_error(
                                '"+" must be between two format selectors', start)
                        current_selector = FormatSelector(
                            MERGE, (video_selector, audio_selector), [])
                    else:
                        raise syntax_error(
                            'Operator not recognized: "{0}"'.format(string), start)
        ...
        def _build_selector_function(selector):
            if isinstance(selector, list):
                ...
                def selector_function(ctx):
                    ...
                    if format_spec == 'all':
                        ...
                        if format_spec in extensions:
                            def filter_f(f): return f['ext'] == format_spec
                        else:
                            def filter_f(
                                f): return f['format_id'] == format_spec
                ...
                video_selector, audio_selector = map(
            ...
            def final_selector(ctx):
                ...
                for _filter in filters:
                    ctx_copy['formats'] = list(
                        filter(_filter, ctx_copy['formats']))
        ...
        try:
            tokens = list(_remove_unused_ops(
                compat_tokenize_tokenize(stream.readline)))
    ...
    def process_video_result(self, info_dict, download=True):
        ...
        for field in ('chapter', 'season', 'episode'):
            if info_dict.get('%s_number' % field) is not None and not info_dict.get(field):
                info_dict[field] = '%s %d' % (
                    field.capitalize(), info_dict['%s_number' % field])
        ...
        for i, format in enumerate(formats):
            ...
            if not format.get('format_id'):
                ...
                format['format_id'] = re.sub(
                    r'[\s,/+\[\]()]', '_', format['format_id'])
        ...
        for i, format in enumerate(formats):
            if format.get('format') is None:
                format['format'] = '{id} - {res}{note}'.format(
                    ...
                    note=' ({0})'.format(format['format_note']) if format.get(
                        'format_note') is not None else '',
                )
        ...
        if req_format is None:
            req_format = self._default_format_spec(
                info_dict, download=download)
            if self.params.get('verbose'):
        ...
        incomplete_formats = (
            ...
            all(f.get('vcodec') != 'none' and f.get(
                'acodec') == 'none' for f in formats)
        ...
        if download:
            if len(formats_to_download) > 1:
                self.to_screen('[info] %s: downloading video in %s formats' % (
    ...
    def process_subtitles(self, video_id, normal_subtitles, automatic_captions):
        ...
        for lang in requested_langs:
            ...
            if formats is None:
                self.report_warning(
                    '%s subtitles not available for %s' % (lang, video_id))
                continue
    ...
    def __forced_printings(self, info_dict, filename, incomplete):
        ...
        if self.params.get('forceurl', False) and not incomplete:
            if info_dict.get('requested_formats') is not None:
                ...
                self.to_stdout(info_dict['url'] +
                               info_dict.get('play_path', ''))
    ...
    def process_info(self, info_dict):
        ...
        def ensure_dir_exists(path):
            try:
                ...
                self.report_error(
                    'unable to create directory ' + error_to_compat_str(err))
                return False
        ...
        if self.params.get('writedescription', False):
            descfn = replace_extension(
                filename, 'description', info_dict.get('ext'))
        ...
        if self.params.get('writeannotations', False):
            annofn = replace_extension(
                filename, 'annotations.xml', info_dict.get('ext'))
        ...
        if subtitles_are_requested and info_dict.get('requested_subtitles'):
            ...
            for sub_lang, sub_info in subtitles.items():
                ...
                if self.params.get('nooverwrites', False) and os.path.exists(encodeFilename(sub_filename)):
                    self.to_screen('[info] Video subtitle %s.%s is already present' % (
                        sub_lang, sub_format))
                else:
                    self.to_screen(
                    ...
                    if sub_info.get('data') is not None:
                        try:
                            ...
                            self.report_error(
                                'Cannot write subtitles file ' + sub_filename)
                            return
                    else:
                        try:
        ...
        if self.params.get('writeinfojson', False):
            ...
            if self.params.get('nooverwrites', False) and os.path.exists(encodeFilename(infofn)):
                ...
                try:
                    write_json_file(
                        self.filter_requested_info(info_dict), infofn)
                except (OSError, IOError):
                    self.report_error(
            ...
            try:
                def dl(name, info):
                    fd = get_suitable_downloader(
                        info, self.params)(self, self.params)
                ...
                self.report_error('content too short (expected %s bytes and served %s)' % (
        ...
        for pp in pps_chain:
            ...
            if files_to_delete and not self.params.get('keepvideo', False):
                for old_filename in files_to_delete:
                    self.to_screen(
                        'Deleting original file %s (pass -k to keep)' % old_filename)
                    try:
    ...
    def list_formats(self, info_dict):
        ...
        table = [
            [f['format_id'], f['ext'], self.format_resolution(
                f), self._format_note(f)]
            for f in formats
    ...
    def list_thumbnails(self, info_dict):
        thumbnails = info_dict.get('thumbnails')
        if not thumbnails:
            self.to_screen('[info] No thumbnails present for %s' %
                           info_dict['id'])
            return
    ...
    def print_debug_header(self):
        ...
        self._write_string('[debug] Proxy map: ' +
                           compat_str(proxy_map) + '\n')

        if self.params.get('call_home', False):
            ipaddr = self.urlopen(
                'https://yt-dl.org/ip').read().decode('utf-8')
    ...
    def _setup_opener(self):
        ...
        self._socket_timeout = 600 if timeout_val is None else float(
            timeout_val)
    ...
    def _write_thumbnails(self, info_dict, filename):
        ...
        for t in thumbnails:
            ...
            t['filename'] = thumb_filename = os.path.splitext(
                filename)[0] + suffix + '.' + thumb_ext
